...
namespace Lzma1900DotNetCore.Common
{
    public class Parser
    {
        ...
        private static readonly char kSwitchID1 = '-';
        private static readonly char kSwitchID2 = '/';

        private static readonly char kSwitchMinus = '-';
        private static readonly string kStopSwitchParsing = "--";
        ...
        public Parser(int numSwitches)
        {
            ...
            for (int i = 0; i < numSwitches; i++) _switches[i] = new SwitchResult();
        }

        public bool ParseString(string srcString, SwitchForm[] switchForms)
        {
            ...
            while (pos < len)
            {
                ...
                if (maxLen == kNoLen) throw new Exception("maxLen == kNoLen");

                SwitchResult matchedSwitch = _switches[matchedSwitchIndex];
                SwitchForm switchForm = switchForms[matchedSwitchIndex];

                if ((!switchForm.Multi) && matchedSwitch.ThereIs) throw new Exception("switch must be single");
                ...
                switch (type)
                {
                    ...
                    case SwitchType.PostChar:
                        {
                            if (tailSize < switchForm.MinLen) throw new Exception("switch is not full");
                    ...
                    case SwitchType.UnLimitedPostString:
                        {
                            ...
                            if (tailSize < minLen) throw new Exception("switch is not full");
        ...
        static bool ParseSubCharsCommand(int numForms, CommandSubCharsSet[] forms, string commandString, ArrayList indices)
        {
            indices.Clear();
            int numUsedChars = 0;
            for (int i = 0; i < numForms; i++)
            {
                ...
                for (int j = 0; j < len; j++)
                {
                    ...
                    if (newIndex >= 0)
                    {
                        ...
                        if (commandString.IndexOf(c, newIndex + 1) >= 0) return false;

                        currentIndex = j;
                        numUsedChars++;
                    }
                }
                if (currentIndex == -1 && !charsSet.EmptyAllowed) return false;
